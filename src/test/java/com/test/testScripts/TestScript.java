...
//			billEntryPage.clickOnNextButton();
			String relatedNumberFromCMS = getData.valueFromHeader("CMS","RelNO",1);
			...
			System.out.println("value: "+lineItemsRowsForCMS.get(counter));
			}
			for(int counter=0; counter<lineItemsRowsForCMS.size();counter++){
//			String relatedNumberFromLineItems = getData.valueFromHeader("LineitemCMS","RelNO",lineItemsRowsForCMS.get(counter));

			fillValuesInLineItemsPage(getData.valueFromHeader("CMS","Type",iter),  lineItemsRowsForCMS.get(counter), counter);
			}
	...
	private void fillValuesInLineItemsPage(String valueFromHeader, int rowData, int position) throws IOException {
		...
		billEntryPage.inputTextIntoPOS(getData.valueFromHeader("CMS","POS",rowData));
		billEntryPage.inputTextIntoTOS(getData.valueFromHeader("CMS","TOS",rowData));

		if(valueFromHeader.equalsIgnoreCase("CMS")){

			billEntryPage.inputTextIntoBillCd(getData.valueFromHeader("CMS","BillCode",rowData));
			billEntryPage.inputTextIntoBillMd(getData.valueFromHeader("CMS","BillMod",rowData));
			...
			billEntryPage.inputTextIntoCMSDays(getData.valueFromHeader("CMS","Days",rowData));
			billEntryPage.inputTextIntoChrgs(getData.valueFromHeader("CMS","Charges",rowData));
			billEntryPage.inputTextIntoDiag(getData.valueFromHeader("CMS","Diag",rowData));
			...
			//billEntryPage.inputTextIntoRevCd(getData.valueFromHeader("CMS","ReviewedCd",rowData));
			//billEntryPage.inputTextIntoRevMd(getData.valueFromHeader("CMS","ReviewedMd",rowData));
			billEntryPage.inputTextIntoUB04Days(getData.valueFromHeader("CMS","Days",rowData));
			billEntryPage.inputTextIntoChrgs(getData.valueFromHeader("CMS","Charges",rowData));
			billEntryPage.inputTextIntoDiag(getData.valueFromHeader("CMS","Diag",rowData));
			billEntryPage.inputTextIntoTotalCharges(getData.valueFromHeader("CMS","TotalCharges",rowData));
		}
